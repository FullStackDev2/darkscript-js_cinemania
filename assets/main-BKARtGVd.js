...
`,$=document.getElementById("selectedCountry");$.insertAdjacentHTML("afterend",re);function D(i,u){const x=i>480?480:i;if(x===0){oe.innerHTML="";return}new ce({containerId:"pagination",totalItems:x,itemsPerPage:20,currentPage:u,onPageChange:M=>{console.log("Sayfa Değişiyor -> Yeni Sayfa:",M),l==="trending"?_(M):j(M);const B=document.getElementById("moviesContainer");if(B){const C=B.getBoundingClientRect().top+window.scrollY-100;window.scrollTo({top:C,behavior:"smooth"})}}})}if(fetch(`${f}/genre/movie/list?api_key=${o}&language=en-US`).then(i=>i.json()).then(i=>i.genres.forEach(u=>s[u.id]=u.name)),I.addEventListener("click",i=>{i.target.closest(".search-clear-btn")&&(S.value="",S.focus())}),I.addEventListener("input",i=>{if(i.target!==S)return;const u=I.querySelector(".search-clear-btn");u&&(u.style.display=S.value?"block":"none")}),ne.addEventListener("click",i=>{i.stopPropagation(),y.classList.toggle("open")}),E.querySelectorAll("li").forEach(i=>{i.addEventListener("click",u=>{u.stopPropagation();const b=i.textContent.trim();$.textContent=b,r=a[b]||"",y.classList.remove("open")})}),document.addEventListener("click",i=>{y.contains(i.target)||y.classList.remove("open")}),E&&!E.dataset.init){const i=document.createElement("li");i.textContent="Country",E.prepend(i),E.dataset.init="true"}T.addEventListener("click",()=>{m.value="",m.focus(),T.style.display="none"}),y.querySelectorAll(".country-list li").forEach(i=>{i.addEventListener("click",()=>{$.textContent=i.textContent,y.classList.remove("open")})}),y.querySelectorAll(".country-list li").forEach(i=>{i.addEventListener("click",()=>{const u=i.textContent.trim();$.textContent=u,r=a[u]||"",y.classList.remove("open")})}),document.addEventListener("click",i=>{const u=document.getElementById("countrySelect");u.contains(i.target)||u.classList.remove("open")}),y&&k&&E&&(k.readOnly=!0,E.querySelectorAll("li").forEach(i=>{i.addEventListener("click",u=>{u.stopPropagation();const b=i.textContent.trim();k.value=b,r=a[b]||"",y.classList.add("has-value"),y.classList.remove("open")})})),h&&v&&(h.addEventListener("click",i=>{i.stopPropagation();const u=v.classList.toggle("open");h.classList.toggle("open",u)}),v.addEventListener("click",i=>{i.stopPropagation(),i.target.tagName==="LI"&&(e=i.target.dataset.year||"",L.textContent=i.target.textContent,v.classList.remove("open"),h.classList.remove("open"))})),document.addEventListener("click",i=>{v.classList.contains("open")&&!v.contains(i.target)&&!h.contains(i.target)&&(v.classList.remove("open"),h.classList.remove("open"))}),p.addEventListener("click",()=>{if(t=m.value.trim(),n=!0,!t&&!e&&!r){n=!1,_();return}j()}),g&&g.addEventListener("click",function(){m.value="",k&&(k.value=""),L.textContent="Year",t="",e="",r="",n=!1,y&&y.classList.remove("has-value"),_()});function j(i=1){let u=`${f}/discover/movie?api_key=${o}&page=${i}`;t&&(u+=`&with_text_query=${encodeURIComponent(t)}`),e&&(u+=`&primary_release_year=${e}`),r&&(u+=`&with_origin_country=${r}`),fetch(u).then(b=>b.json()).then(b=>{O(b.results||[]),D(b.total_results,i)})}_(1);function _(i=1){d.style.display="none",l="trending",fetch(`${f}/trending/movie/week?api_key=${o}&page=${i}`).then(u=>u.json()).then(u=>{O(u.results||[]),D(u.total_results,i)})}function O(i){if(c.innerHTML="",!i.length){d.style.display=n?"block":"none";return}d.style.display="none",i.slice(0,10).forEach(u=>{var M,B;const b=document.createElement("a");b.className="movie-card",b.setAttribute("data-id",u.id),b.href=`catalog_mainbody.html?id=${u.id}`;const R=u.poster_path?`${H}${u.poster_path}`:"https://via.placeholder.com/300x450",G=((M=u.release_date)==null?void 0:M.slice(0,4))||"N/A",x=((B=u.genre_ids)==null?void 0:B.map(C=>s[C]).filter(Boolean).slice(0,2).join(", "))||"Unknown";b.innerHTML=`
        <img src="${R}">
        <div class="movie-card-overlay">
          <div class="movie-card-text">
            <h3>${u.title}</h3>
    ...
    `}}function be(e){return e[Math.floor(Math.random()*e.length)]}async function Le(){const e=document.getElementById("hero-section");if(!e)return;const t=e.getAttribute("data-page");if(t!=="library"&&t==="dynamic")try{const r=await he("day");if(r&&r.length>0){const n=be(r);we(e,n)}else J(e)}catch(r){console.error("Hero yüklenirken hata:",r),J(e)}}document.addEventListener("DOMContentLoaded",()=>{Le().catch(e=>console.error("initHero error:",e))});function we(e,t){const{title:r,overview:n,backdrop_path:s,vote_average:l,id:a}=t,o=()=>{const c="linear-gradient(90deg, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.65) 20%, rgba(0,0,0,0.0) 60%)",d=window.devicePixelRatio>1.1;let m="";if(s){let g="w780";window.innerWidth>=1280?g=d?"original":"w1280":window.innerWidth>=768&&(g=d?"w1280":"w780"),m=`https://image.tmdb.org/t/p/${g}${s}`}else m=`./background/desktop-1${d?"-@2x":""}.jpg`;e.style.backgroundImage=`${c}, url('${m}')`,e.style.backgroundSize="cover",e.style.backgroundPosition="center"};o(),new MutationObserver(o).observe(document.body,{attributes:!0,attributeFilter:["class"]}),window.addEventListener("resize",o),e.innerHTML=`
...
//# sourceMappingURL=main-BKARtGVd.js.map
